...
use bridge_ctypes::{external_to_baml_value, HandleTableOptions};
...
pub fn send_result_to_callback(id: u32, value: &BexExternalValue) {
    let callback_fn = match CALLBACK_FN.get() {
        Some(f) => f,
        None => {
            eprintln!("bridge_go: callback not registered");
            return;
        }
    };

    let handle_options = HandleTableOptions::for_in_process();
...
pub fn send_error_to_callback(id: u32, error: &str) {
    let callback_fn = match CALLBACK_FN.get() {
        Some(f) => f,
        None => {
            eprintln!("bridge_go: callback not registered: {error}");
            return;
        }
    };
    tokio::task::block_in_place(|| {
