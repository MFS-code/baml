use std::ffi::CStr;
use std::panic::AssertUnwindSafe;

use bridge_cffi::engine::{get_runtime, get_tokio_runtime};
use bridge_ctypes::{kwargs_to_bex_values, DecodeFromBuffer, HANDLE_TABLE};
use futures::FutureExt;

use crate::callbacks::{send_error_to_callback, send_result_to_callback};
use crate::errors::bridge_error_to_string;

/// Cancel an in-flight function call by firing the CancellationToken for the
/// given call ID.  This causes the engine to abort any spawned async tasks
/// (HTTP requests, sleeps, etc.) and return EngineError::Cancelled.
///
/// Returns 0 on success, 1 if the call ID is unknown or already completed.
#[unsafe(no_mangle)]
pub extern "C" fn cancel_function_call(id: u32) -> i32 {
    let runtime = match get_runtime() {
        Ok(rt) => rt,
        Err(_) => return 1,
    };
    match runtime.cancel_function_call(sys_types::CallId(id.into())) {
        Ok(()) => 0,
        Err(_) => 1,
    }
}

#[unsafe(no_mangle)]
pub extern "C" fn call_function(
    function_name: *const libc::c_char,
    encoded_args: *const u8,
    length: usize,
    id: u32,
) {
    if let Err(e) = call_function_inner(function_name, encoded_args, length, id) {
        send_error_to_callback(id, &bridge_error_to_string(&e));
    }
}

fn call_function_inner(
    function_name: *const libc::c_char,
    encoded_args: *const u8,
    length: usize,
    id: u32,
) -> Result<(), bridge_cffi::BridgeError> {
    use bridge_cffi::error::BridgeError;
    use bridge_ctypes::baml::cffi::CallFunctionArgs;

    let runtime = get_runtime()?;

    if function_name.is_null() {
        return Err(BridgeError::NullFunctionName);
    }
    let func_name = unsafe { CStr::from_ptr(function_name) }
        .to_str()
        .map_err(BridgeError::InvalidFunctionName)?
        .to_owned();

    let args = if encoded_args.is_null() || length == 0 {
        CallFunctionArgs::default()
    } else {
        unsafe { CallFunctionArgs::from_c_buffer(encoded_args, length) }?
    };
    let kwargs = kwargs_to_bex_values(args.kwargs, &HANDLE_TABLE)?;

    let call_ctx =
        bex_project::FunctionCallContextBuilder::new(sys_types::CallId(id.into()));

    let rt = get_tokio_runtime()?;
    rt.spawn(async move {
        let result = AssertUnwindSafe(
            runtime.call_function(&func_name, kwargs.into(), call_ctx.build()),
        )
        .catch_unwind()
        .await;

        match result {
            Ok(Ok(value)) => {
                send_result_to_callback(id, &value);
            }
            Ok(Err(e)) => {
                send_error_to_callback(id, &format!("{e}"));
            }
            Err(panic_info) => {
                let msg = if let Some(s) = panic_info.downcast_ref::<&str>() {
                    s.to_string()
                } else if let Some(s) = panic_info.downcast_ref::<String>() {
                    s.clone()
                } else {
                    "Unknown panic".to_string()
                };
                send_error_to_callback(id, &format!("Panic: {msg}"));
            }
        }
    });

    Ok(())
}
